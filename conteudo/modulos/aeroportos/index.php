   ...
   { Switch($Acao)
     {
       ...
       Case 'Multipla-Ativar':
            ...
            If ($_SERVER['REQUEST_METHOD'] == 'POST' && isSet($_POST['Selecao']) && !empty($_POST['Selecao']))
               { $Acao       = Aeroporto_Acoes($_POST['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
       ...
       Case 'Multipla-Excluir':
            ...
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && isSet($_SESSION['Selecao']) && isSet($_GET['pOK']))
               { $Acao       = Aeroporto_Acoes($_SESSION['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
...
$_Filtros->Adicionar($i++, 'Tipo', 'Tipo', 'Texto', OPT_Comum('Aeroporto_Tipo', False));
$_Filtros->Adicionar($i++, 'Continente', 'Continente_ISO', 'Texto', $_Mundo->Continentes(True));
$_Filtros->Adicionar($i++, 'País', 'Pais_ISO', 'Texto', $_Mundo->Paises(False, True));
$_Filtros->Adicionar($i++, 'Bases', 'Bases', 'Boleano', OPT_Comum('Sim_Nao', False));
...
$_Ordem->Adicionar($i++, 'IATA [A-z]', 'IATA ASC');
$_Ordem->Adicionar($i++, 'IATA [Z-a]', 'IATA DESC');
$_Ordem->Adicionar($i++, 'Cidade [A-z]', 'Cidade ASC');
$_Ordem->Adicionar($i++, 'Cidade [Z-a]', 'Cidade DESC');
$_Ordem->Adicionar($i++, 'País [A-z]', 'Pais ASC');
$_Ordem->Adicionar($i++, 'País [Z-a]', 'Pais DESC');
...
$aCP['Contato']    = Array('Tamanho' => '5%', 'Campo' => '$Contato');
$aCP['Local']      = Array('Tamanho' => '25%', 'Campo' => '$Local');
...
If ($Exibindo > 0)
   { $Registros    = '';
     ...
     Foreach($RG as $Linha => $Dados) {
       ...
       $Objeto               = 'o aeroporto [ '.$Dados['Nome'].' ]';
       $Status               = $Dados['Reg_Status'];
       $aDados['Registro']   = $Status.'<br>'.$_TP->Data($Dados['Reg_CadData']);
       $aDados['Primario']   = '<div class="Lista_DivEsq">'.$Dados['Nome'].'</div><div class="Lista_DivDes Lista_DivDir">'.Vazio($Dados['Cidade']).' - <b>'.$Dados['IATA'].'</b></div>';
       $aDados['Tipo']       = $Dados['Tipo'].( $Dados['Tamanho'] ? ' '.$Dados['Tamanho'] : '' );
       $aDados['Contato']    = '-';
       $Imagem               = is_file('midia/img/paises/'.strtolower($Dados['Pais_ISO']).'.png') ? 'midia/img/paises/'.strtolower($Dados['Pais_ISO']).'.png' : '';
       $aDados['Local']      = $Imagem ? '<div class="Lista_DivEsq">'.Vazio($Dados['Regiao']).'</div><div class="Lista_DivDir">'.$Dados['Pais'].' <img class="Cursor" src="'.$Imagem.'" title="'.$Dados['Pais_ISO'].'"></div>' : '<div class="Lista_DivEsq">'.Vazio($Dados['Estado']).'</div><div class="Lista_DivDir"><b>'.$Dados['Pais'].'</b></div>';
