   ...
   { Switch($Acao)
     {
       ...
       Case 'Multipla-Ativar':
            ...
            If ($_SERVER['REQUEST_METHOD'] == 'POST' && isSet($_POST['Selecao']) && !empty($_POST['Selecao']))
               { $Acao       = Jogador_Acoes($_POST['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
       ...
       Case 'Multipla-Excluir':
            ...
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && isSet($_SESSION['Selecao']) && isSet($_GET['pOK']))
               { $Acao       = Jogador_Acoes($_SESSION['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
...
$_Filtros->Adicionar($i++, 'Tipo', 'Tipo', 'Texto', OPT_Comum('Jogador_Tipo', False));
$_Filtros->Adicionar($i++, 'Acesso', 'Acesso', 'Boleano', OPT_Comum('Sim_Nao', False));
$_Filtros->Adicionar($i++, 'Estado', 'End_Estado', 'Texto', OPT_Estados(False));
$_Filtros->Adicionar($i++, 'Região', 'End_Regiao', 'Texto', OPT_Regioes(False));
...
// Opcoes
$_Pesquisa->Adicionar($i++, $Modulo, 'Tratamento', 'Texto');
$_Pesquisa->Adicionar($i++, 'CPF', 'CPF', 'Texto');
$_Pesquisa->Adicionar($i++, 'CNPJ', 'CNPJ', 'Texto');
$_Pesquisa->Adicionar($i++, 'E-mail', 'Email', 'Texto');
$_Pesquisa->Adicionar($i++, 'Telefone', 'Tel1', 'Texto');
$_Pesquisa->Adicionar($i++, 'Nascimento', 'Nascimento', 'Data');
$_Pesquisa->Adicionar($i++, 'Registro', 'Reg_CadData', 'Data', 'Periodo');
$_Pesquisa->Adicionar($i++, 'Nascimento', 'Nascimento', 'Data', 'Periodo');
...
// Opcoes
$_Ordem->Adicionar($i++, 'Nome [A-z]', 'Tratamento ASC', True);
$_Ordem->Adicionar($i++, 'Nome [Z-a]', 'Tratamento DESC');
...
$aCP['Tipo']       = Array('Tamanho' => '20%', 'Campo' => '$Tipo');
$aCP['Contato']    = Array('Tamanho' => '23%', 'Campo' => '$Contato');
$aCP['Local']      = Array('Tamanho' => '7%', 'Campo' => '$Local');
...
If ($Exibindo > 0)
   { $Registros    = '';
     ...
     Foreach($RG as $Linha => $Dados) {
       ...
       $aCliente             = Jogador_Nome($Dados, 'Array');
       $Objeto               = 'o jogador [ '.$aCliente['Tratamento'].' ]';
       $Status               = $Dados['Reg_Status'];
       $aDados['Registro']   = $Status.'<br>'.$_TP->Data($Dados['Reg_CadData']);
       $aDados['Primario']   = '<div class="Lista_DivDes">'.$aCliente['Nome'].'</div>'.( $aCliente['Tratamento'] != $aCliente['Nome'] ? '<div class="Lista_DivDir">'.$aCliente['Tratamento'].'</div>' : '');
       $aDados['Tipo']       = $Dados['Tipo'] == 'Pessoa Jurídica' ? '<div class="Lista_DivEsq">'.$Dados['Tipo'].'</div><div class="Lista_DivDir">CNPJ: '.Formata_CNPJ($Dados['CNPJ']).'</div>' : '<div class="Lista_DivEsq">'.$Dados['Tipo'].'</div><div class="Lista_DivDir">CPF: '.Formata_CPF($Dados['CPF']).'</div>';
       $aDados['Contato']    = '<div class="Lista_DivEsq">'.Vazio($Dados['Email']).'</div><div class="Lista_DivDir">'.Formata_Tel($Dados['Tel']).'</div>';
       $aDados['Local']      = Vazio($Dados['End_Estado']).'<br>'.Vazio($Dados['End_Regiao']).'</div>';
