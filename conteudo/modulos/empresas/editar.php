   ...
   { $SQL          = 'SELECT * FROM '.$Tabela.' WHERE Codigo = $1';
     ...
     If ($Registro = $_DB->Registros($Consulta))
        { $RG      = $_DB->Dados($Consulta);
          ...
          List( $Tel, $Tel_DDD )       = empty($RG['Tel']) ? Array('', ( defined('EMPRESA_DDD') ? EMPRESA_DDD : '' )) : Array(substr($RG['Tel'], 2), substr($RG['Tel'], 0, 2));
          List( $Tel2, $Tel2_DDD )     = empty($RG['Tel2']) ? Array('', ( defined('EMPRESA_DDD') ? EMPRESA_DDD : '' )) : Array(substr($RG['Tel2'], 2), substr($RG['Tel2'], 0, 2));
          List( $Tel3, $Tel3_DDD )     = empty($RG['Tel3']) ? Array('', ( defined('EMPRESA_DDD') ? EMPRESA_DDD : '' )) : Array(substr($RG['Tel3'], 2), substr($RG['Tel3'], 0, 2));
          // Options
...
// LISTA
$Lista             = Empresa_OPT($_ID, True, 'Codigo, Nome, Tipo', '$Nome ($Tipo)', 'Codigo', 'Nome ASC', '0');
...
<?php
// ID
If ( $Usar_ID ) Echo '                  <div class="Form_Linha"><div class="Form_Esq">ID:</div><div class="Form_Dir"><input class="Form_Text" type="text" name="ID" id="" value="'.( isSet($_POST['ID']) ? $_POST['ID'] : $RG['ID'] ).'" maxlength="20" style="width:140px;"> * Se vazio o sistema gerará um código</div></div>'.PHP_EOL;
