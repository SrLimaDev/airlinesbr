...
// Fabricantes
If (is_file(SIS_Dir_CFG_Modulos.'/fabricantes.cfg.php')) include_once(SIS_Dir_CFG_Modulos.'/fabricantes.cfg.php');
If (is_file(SIS_Dir_Funcoes_Modulos.'/fabricantes.php')) include_once(SIS_Dir_Funcoes_Modulos.'/fabricantes.php');
   ...
   { Switch($Acao)
     {
       ...
       Case 'Multipla-Ativar':
            ...
            If ($_SERVER['REQUEST_METHOD'] == 'POST' && isSet($_POST['Selecao']) && !empty($_POST['Selecao']))
               { $Acao       = Aviao_Acoes($_POST['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
       ...
       Case 'Multipla-Excluir':
            ...
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && isSet($_SESSION['Selecao']) && isSet($_GET['pOK']))
               { $Acao       = Aviao_Acoes($_SESSION['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
...
// Opcoes
$_Filtros->Adicionar($i++, 'Status', 'Reg_Status', 'Texto', OPT_Comum('Status', False));
$_Filtros->Adicionar($i++, 'Tipo', 'Tipo', 'Texto', OPT_Comum('Aviao_Tipo', False));
$_Filtros->Adicionar($i++, 'Alcance', 'Alcance', 'Texto', OPT_Comum('Aviao_Alcance', False));
//$_Filtros->Adicionar($i++, 'Fabricante', 'Fabricante', 'Inteiro', Fabricante_OPT(0, False, 'Codigo, Nome', '$Nome', 'Codigo', 'Nome ASC', '0'));
...
$_Pesquisa->Adicionar($i++, 'IATA', 'IATA', 'Texto');
$_Pesquisa->Adicionar($i++, 'Cidade', 'Cidade', 'Texto');
...
$_Ordem->Adicionar($i++, 'Passageiros [-]', 'Assentos ASC');
$_Ordem->Adicionar($i++, 'Passageiros [+]', 'Assentos DESC');
$_Ordem->Adicionar($i++, 'Carga [-]', 'Carga ASC');
$_Ordem->Adicionar($i++, 'Carga [+]', 'Carga DESC');
$_Ordem->Adicionar($i++, 'Alcance Mínimo [-]', 'Alcance_Minimo ASC');
$_Ordem->Adicionar($i++, 'Alcance Mínimo [+]', 'Alcance_Minimo DESC');
$_Ordem->Adicionar($i++, 'Alcance Máximo [-]', 'Alcance_Maximo ASC');
$_Ordem->Adicionar($i++, 'Alcance Máximo [+]', 'Alcance_Maximo DESC');
$_Ordem->Adicionar($i++, 'Velocidade [-]', 'Velocidade ASC');
$_Ordem->Adicionar($i++, 'Velocidade [+]', 'Velocidade DESC');
$_Ordem->Adicionar($i++, 'Consumo [-]', 'Combustivel ASC');
$_Ordem->Adicionar($i++, 'Consumo [+]', 'Combustivel DESC');
...
$Extra             = $Modulo.':Contato'; // Campos que usaram o espaco extra de 10% (que sobra) no relatório
$aCP               = Array(); // Campos
$aCP[$Primario]    = Array('Tamanho' => '30%', 'Campo' => '$Primario');
$aCP['Tipo']       = Array('Tamanho' => '15%', 'Campo' => '$Tipo');
$aCP['Alcance']    = Array('Tamanho' => '20%', 'Campo' => '$Alcance');
$aCP['Velocidade'] = Array('Tamanho' => '15%', 'Campo' => '$Velocidade');
...
If ($Exibindo > 0)
   { $Registros    = '';
     ...
     Foreach($RG as $Linha => $Dados) {
       ...
       $Objeto               = 'o avião [ '.$Dados['Nome'].' ]';
       $Status               = $Dados['Reg_Status'];
       $aDados['Registro']   = $Status.'<br>'.$_TP->Data($Dados['Reg_CadData']);
       $aDados['Primario']   = '<div class="Lista_DivDes">'.$Dados['Nome'].'</div>'.( $Dados['Fabricante'] ? '<div class="Lista_DivDes Lista_DivDir">'.$Dados['Fabricante_Nome'].'</div>' : '' );
       $aDados['Tipo']       = '<div class="Lista_DivEsq"><b>'.$Dados['Tipo'].'</b></div><div class="Lista_DivDir">'.( $Dados['Tipo'] == 'Carga' ? Formata_Medida($Dados['Carga'], 'kg') : $Dados['Assentos'].' Assentos' ).'</div>';
       $aDados['Alcance']    = '<b>'.$Dados['Alcance'].'</b><br>'.Formata_Distancia($Dados['Alcance_Minimo'], 'km').' à '.Formata_Distancia($Dados['Alcance_Maximo'], 'km');
       $aDados['Velocidade'] = '<div class="Lista_DivEsq">'.$Dados['Velocidade'].' Km/H</div><div class="Lista_DivDir">'.$Dados['Combustivel'].' L/Km</div>';
...
Else { $Registros  = '                  <tr class="Lista_Linha"><td class="Lista_Cel" colspan="'.($iCP + 2).'" style="height:25px;">'.(isSet($Pesquisa['Registros']) || !empty($pFT_Mensagem) ? 'A pesquisa e/ou fitro(s) aplicados não retornaram resultados' : 'Não encontramos nenhum registro para o módulo '.$Modulos).'</td></tr>'.PHP_EOL; }
