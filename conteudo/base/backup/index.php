   ...
   { Switch($Acao)
     {
       ...
       /*** COMPRESSAO ***/
       Case 'Compactar':
            $Acao_Nome       = 'Compressão de '.$Modulo;
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && !empty($_ID))
               { $Acao       = Backup($_ID, $Acao);
                 $MEN[$Acao['RT']]     = '<b>'.$Acao_Nome.'</b>:<br>'.$Acao['Info'];
               }
       ...
       Case 'Multipla-Compactar':
            $Acao_Nome       = 'Compressão múltipla de '.$Modulos;
            If ($_SERVER['REQUEST_METHOD'] == 'POST' && isSet($_POST['Selecao']) && !empty($_POST['Selecao']))
               { $MEN['Aviso']         = '<b>'.$Acao_Nome.'</b> (confirmação necessária):<br>Tem certeza que deseja compactar todos os registros selecionados? <b><a id="Azul" href="'.URL(True, True).'&pOK='.Rand_Numero(5).Rand_Letra(5).Rand_Numero(5).'">Sim</a></b> ou <b><a id="Vermelho" href="'.$_URL_Pag.'">Não</a></b><script>setTimeout("IrPara(\''.$_URL_Pag.'\');", '.($TP_Confirmacao * 1000).');</script>';
                 $_SESSION['Selecao']  = $_POST['Selecao'];
               } Else
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && isSet($_SESSION['Selecao']) && isSet($_GET['pOK']))
               { $Acao       = Backup_Acoes($_SESSION['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
       ...
       /*** DESCOMPRESSAO ***/
       Case 'Descompactar':
            $Acao_Nome       = 'Descompressão de '.$Modulo;
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && !empty($_ID))
               { $Acao       = Backup($_ID, $Acao);
                 $MEN[$Acao['RT']]     = '<b>'.$Acao_Nome.'</b>:<br>'.$Acao['Info'];
               }
       ...
       Case 'Multipla-Descompactar':
            $Acao_Nome       = 'Descompressão múltipla de '.$Modulos;
            If ($_SERVER['REQUEST_METHOD'] == 'POST' && isSet($_POST['Selecao']) && !empty($_POST['Selecao']))
               { $MEN['Aviso']         = '<b>'.$Acao_Nome.'</b> (confirmação necessária):<br>Tem certeza que deseja descompactar todos os registros selecionados? <b><a id="Azul" href="'.URL(True, True).'&pOK='.Rand_Numero(5).Rand_Letra(5).Rand_Numero(5).'">Sim</a></b> ou <b><a id="Vermelho" href="'.$_URL_Pag.'">Não</a></b><script>setTimeout("IrPara(\''.$_URL_Pag.'\');", '.($TP_Confirmacao * 1000).');</script>';
                 $_SESSION['Selecao']  = $_POST['Selecao'];
               } Else
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && isSet($_SESSION['Selecao']) && isSet($_GET['pOK']))
               { $Acao       = Backup_Acoes($_SESSION['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
...
$_Filtros->Adicionar($i++, 'Compactação', 'ZIP', 'Boleano', OPT_Comum('Sim_Nao', False));
$_Filtros->Adicionar($i++, 'Cópia FTP', 'FTP', 'Boleano', OPT_Comum('Sim_Nao', False));
            ...
            <div class="Sep_Hor"></div>
              <div class="Corpo_Ferramentas">
                ...
                <div class="Ferramentas">
                  ...
                  <div class="Sep_Hor"></div>
                  <img src="midia/img/fer_cfg.png" class="Cursor" onClick="IrPara('<?php Echo URL_Link('Sistema:Configuracoes', 0, 0, 'cID='.Texto_SEO($Modulo, False)); ?>');" title="Configurações">
                  <div class="Sep_Hor"></div>
                  <img class="Cursor" src="midia/img/fer_tamanho.gif" width="30" title="Espaço Total Ocupado: <?php Echo Backup_Espaco(); ?> - Clique para Detalhes" onClick="Backup_Espaco('<?php Echo Backup_Espaco('Explode'); ?>');">
                </div>
                <!-- Filtros -->
                <div class="Filtro">
...
$Extra             = $Modulo; // Campos que usaram o espaco extra de 10% (que sobra) no relatório
$aCP               = Array(); // Campos
...
If ($Exibindo > 0)
   { $Registros    = '';
     ...
     Foreach($RG as $Linha => $Dados) {
       ...
       $aDados['ZIP']        = $Dados['ZIP'] ? 'Sim' : 'Não';
       $aDados['FTP']        = $Dados['FTP'] ? 'Sim' : 'Não';
       ...
       $BT_FTP               = $Dados['FTP'] ? '<a href="'.URL_Base().'/sis.php?pAC=Download:FTP&ID='.$Codigo.'"><img src="midia/img/bt_download_ftp_on.png" title="Baixar o arquivo d'.$Objeto.'"></a>' : '<img class="Cursor" src="midia/img/bt_download_ftp_off.png" title="Não existe cópia FTP d'.$Objeto.'">';
