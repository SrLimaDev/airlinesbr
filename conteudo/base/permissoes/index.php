...
// OPCOES MULTIPLAS
$Multiplas         = Array('Excluir' => 'Excluir');
   ...
   { Switch($Acao)
     {
       ...
       Case 'Multipla-Excluir':
            ...
            If ($_SERVER['REQUEST_METHOD'] == 'GET' && isSet($_SESSION['Selecao']) && isSet($_GET['pOK']))
               { $Acao       = Permissao_Acoes($_SESSION['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
...
// Opcoes
$_Filtros->Adicionar($i++, 'Fonte', 'Fonte', 'Texto', OPT_Comum('Permissao_Fonte', False));
$_Filtros->Adicionar($i++, 'Módulo', 'Modulo', 'Texto', 'SELECT DISTINCT Modulo FROM '.$Tabela.' ORDER BY Modulo ASC');
$_Filtros->Adicionar($i++, 'Validação', 'Regra', 'Nivel', Permissao_Regras(False));
...
$_Pesquisa->Adicionar($i++, 'ID', 'Acao', 'Texto');
...
// Opcoes
$_Ordem->Adicionar($i++, 'Acao [-]', 'Acao ASC', true);
$_Ordem->Adicionar($i++, 'Acao [+]', 'Acao DESC');
            ...
            <div class="Sep_Hor"></div>
              <div class="Corpo_Ferramentas">
                ...
                <div class="Ferramentas">
                  ...
                  <div class="Sep_Hor"></div>
                  <img src="midia/img/fer_atualizar.png" class="Cursor" onclick="IrPara('<?php Echo URL_Link('Sistema:Permissoes', 'Atualizar'); ?>');" title="Atualizar Sistema">
                </div>
                <!-- Filtros -->
                <div class="Filtro">
...
$Extra             = $Modulo.':Validação'; // Campos que usaram o espaco extra de 10% (que sobra) no relatório
$aCP               = Array(); // Campos
$aCP['ID']         = Array('Tamanho' => '5%', 'Campo' => 'Acao');
$aCP[$Primario]    = Array('Tamanho' => '30%', 'Campo' => '$Primario');
$aCP['Módulo']     = Array('Tamanho' => '15%', 'Campo' => 'Modulo');
$aCP['Validação']  = Array('Tamanho' => '27%', 'Campo' => '$Validacao');
$aCP['Registro']   = Array('Tamanho' => '13%', 'Campo' => '$Registro');
...
If ($Exibindo > 0)
   { $Registros    = '';
     ...
     $Niveis       = SIS_Niveis($_SESSION['SIS']['L']['Tipo'], False, True);

     Foreach($RG as $Linha => $Dados) {
       // Variaveis
       $Codigo               = Formata_Codigo($Dados['Codigo']);
       $Objeto               = 'a permissão [ '.$Dados['Nome'].' - '.$Dados['Modulo'].' ]';
       $Usuario              = SIS_Usuario($Dados['Reg_CadUsuario'], 'Nome'); //if ($Dados['Codigo'] == 1) a($Usuario);
       $aDados['Registro']   = $_TP->Data($Dados['Reg_CadData']).'<br>'.$Usuario;
       $aDados['Primario']   = '<div class="Lista_DivDes">'.$Dados['Nome'].'</div>';
       // Forma
       $Regra                = SIS_Decode_Nivel($Dados['Regra']);
       $Regra_Nivel          = SIS_Decode_Nivel($Dados['Regra_Nivel']);
       Switch($Regra) {
         Case 0: $aDados['Validacao']  = 'Acesso Liberado'; Break;
         Case 1: $aDados['Validacao']  = '<div class="Lista_DivEsq">Nível Mínimo:</div><div class="Lista_DivDir">'.( $Regra_Nivel == max($Niveis) ? array_search($Regra_Nivel, $Niveis) : $Regra_Nivel.' - '.array_search($Regra_Nivel, $Niveis) ).'</div>'; Break;
         Case 2: $aDados['Validacao']  = '<div class="Lista_DivEsq">Nível Específico:</div><div class="Lista_DivDir">'.( $Regra_Nivel == max($Niveis) ? array_search($Regra_Nivel, $Niveis) : $Regra_Nivel.' - '.array_search($Regra_Nivel, $Niveis) ).'</div>'; Break;
         Case 3: $aDados['Validacao']  = 'Nível do proprietário deve ser inferior'; Break;
         Case 4:
              $Usuario                 = SIS_Usuario($Dados['Regra_Usuario'], 'Array');
              $aDados['Validacao']     = '<div class="Lista_DivEsq">Usuário Específico:</div><div class="Lista_DivDir">'.$Usuario['Nome'].' ('.$Usuario['Nivel_Nome'].')</div>';
              Break;
         Case 5: $aDados['Validacao']  = 'Usuário Proprietário'; Break;
         Case 6: $aDados['Validacao']  = 'Responsável pelo Registro'; Break;
         Case 7: $aDados['Validacao']  = 'Condição Especial'; Break;
         Default: $aDados['Validacao'] = '<b style="color:red;">Erro</b>'; Break;
       }
       ...
       // Impressão
       $Atual      = '                  <tr class="'.$Classe.'" id="l'.$Linha.'"><td class="Lista_Cel_Cinza"><input type="checkbox" id="s'.$Linha.'" name="Selecao['.$Linha.']" value="'.$Dados['Codigo'].'"'.$Selecao.' onClick="Selecionar_Linha('.$Linha.');"></td><td class="Lista_Cel_Cinza">'.$Acoes.'</td>';
       $TMP        = '';
       $i          = 0;
