   ...
   { Switch($Acao)
     {
       ...
       Case 'Multipla-Excluir':
            ...
            If ( $_SERVER['REQUEST_METHOD'] == 'GET' && isSet($_SESSION['Selecao']) && isSet($_GET['pOK']) )
               { $Acao       = Log_Acoes($_SESSION['Selecao'], substr($Acao, strpos($Acao, '-') + 1));
...
// Opcoes
$_Filtros->Adicionar($i++, 'Tipo', 'Tipo', 'Texto', OPT_Comum('Log', False));
$_Filtros->Adicionar($i++, 'Backup', 'Backup', 'Boleano', OPT_Comum('Sim_Nao', False));
            ...
            <div class="Sep_Hor"></div>
              <div class="Corpo_Ferramentas">
                ...
                <div class="Ferramentas">
                  ...
                  <div class="Sep_Hor"></div>
                  <img class="Cursor" src="midia/img/fer_tamanho.gif" width="30" title="Espaço Total Ocupado: <?php Echo Log_Espaco(); ?> - Clique para Detalhes" onClick="Log_Espaco('<?php Echo Log_Espaco('Explode'); ?>');">
                </div>
                <!-- Filtros -->
                <div class="Filtro">
...
$Extra             = $Modulo.':Arquivo'; // Campos que usaram o espaco extra de 10% (que sobra) no relatório
$aCP               = Array(); // Campos
$aCP['Tipo']       = Array('Tamanho' => '10%', 'Campo' => 'Tipo');
$aCP[$Primario]    = Array('Tamanho' => '48%', 'Campo' => '$Primario');
$aCP['Tamanho']    = Array('Tamanho' => '10%', 'Campo' => '$Tamanho');
$aCP['Data']       = Array('Tamanho' => '10%', 'Campo' => '$Data');
$aCP['ZIP']        = Array('Tamanho' => '5%', 'Campo' => '$ZIP');
$aCP['Backup']     = Array('Tamanho' => '5%', 'Campo' => '$Backup');
...
// TITULO
$Titulo            = '<tr class="Lista_Linha"><td class="Lista_Cel_Cinza" style="width:3%;"><input type="checkbox" name="Selecao_Total" onClick="Selecionar(this.form)"></td><td class="Lista_Cel_Cinza" style="width:9%;">-</td>';
...
If ($Exibindo > 0)
   { $Registros    = '';
     ...
     Foreach($RG as $Linha => $Dados) {
       // Tamanho
       If ( $Dados['Tipo'] == 'Diário' )
          { $Tamanho         = Arquivo_Tamanho($Dados['Arquivo_URL']);
            $SQL             = 'UPDATE '.$Tabela.' SET Tamanho = "'.$Tamanho['String'].'", Tamanho_Bytes = '.$Tamanho['Bytes'].' WHERE Codigo = '.$Dados['Codigo'];
            $_DB->Consulta($SQL);
          } Else { $Tamanho  = False; }

       // Variaveis
       $Codigo               = Formata_Codigo($Dados['Codigo']);
       $Objeto               = 'o log [ '.$Dados['Nome'].' ]';
       ...
       $aDados['Backup']     = $Dados['Backup'] ? 'Sim' : 'Não';
       $aDados['Tamanho']    = $Tamanho ? $Tamanho['String'] : ( $Dados['Tamanho'] ? $Dados['Tamanho'] : '-' );

       // Acoes
       $BT_Ver               = '<a href="'.URL_Link($_GET['pCT'], 'Ver', 0, $_RG.'='.$Codigo).'"><img src="midia/img/bt_visualizar.png" title="Vizualizar '.$Objeto.'"></a>';
       ...
       $Acoes                = $BT_Ver.' '.$BT_Download.' '.$BT_Excluir;
