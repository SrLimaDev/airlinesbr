 ...
 *********************/
        ...
        // Suporte a IMAGES com onClick = function()
        $("img").each(function() {
			var obj = $(this);
			currentonSubmit = obj.attr("onClick");
			if (currentonSubmit === undefined) {
				currentonSubmit = "";
			}
			obj.attr("onClick", currentonSubmit + "; userMovingWithinSite();");
		});
		
		// Suporte aos INPUTS (como button) com onClick = function()
        $("input").each(function() {
			var obj = $(this);
			var linkIsInternal = false;
			...
			for (i = 0; i < myInternalURLs.length; i++) {
                ...
                if (obj.attr("onClick") && obj.attr("onClick").indexOf(myInternalURLs[i]) !== -1) {
					linkIsInternal = true;
				}
				// if it's a relative or absolute URL, so it's internal.
				if (obj.attr("onClick") && obj.attr("onClick").indexOf("http://") == -1) {
					linkIsInternal = true;
				}
			}
