...
/*** ACOES INDIVIDUAIS ***/
Function Aeroporto($ID = 0, $Acao = 'Dados') { Global $_DB;

$Modulo            = 'Aeroportos';
$Objeto            = 'o aeroporto';

// IDENTIFICACAO
$SQL               = 'SELECT * FROM '.TAB_Aeroportos.' WHERE Codigo = $1';
...
If ( $_DB->Registros($Consulta) )
   { $RG           = $_DB->Dados($Consulta);
     ...
     Switch($Acao) {
       ...
       Case 'Ativar':
            If ( $Status == 'Inativo' )
               { $SQL        = 'UPDATE '.TAB_Aeroportos.' SET Reg_Status = "Ativo" WHERE Codigo = $1';
       ...
       Case 'Desativar':
            If ( $Status == 'Ativo' )
               { $SQL        = 'UPDATE '.TAB_Aeroportos.' SET Reg_Status = "Inativo" WHERE Codigo = $1';
                               $_DB->Consulta($SQL, Array('$1' => $ID));
                 $Log        = Array('Modulo' => $Modulo, 'Evento' => $_SESSION['SIS']['L']['Nome'].' ativou o registro d'.$Objeto.' [ '.$Registro.' ]');
       ...
       // Excluir
       Case 'Excluir':
            $SQL   = 'DELETE FROM '.TAB_Aeroportos.' WHERE Codigo = $1';
                     $_DB->Consulta($SQL, Array('$1' => $ID));
...
/*** ACOES MULTIPLAS ***/
Function Aeroporto_Acoes($IDs = Array(), $Acao = 'Dados') {
...
Function Aeroporto_Cadastrar($POST) { Global $_DB, $_TP, $_SIS, $_Mundo; array_map('trim', $POST);
...
// DUPLICIDADE
$SQL               = 'SELECT Codigo, Codigo_ID AS ID, Nome, IATA, IATA_Nome, IATA_ID FROM '.TAB_Aeroportos.' WHERE Nome = $1 OR IATA = $2 OR IATA_ID = $3'.( $ID ? ' OR ID = $4' : '' );
...
If ( $_DB->Registros($Consulta) )
   { $TMP          = $_DB->Dados_Array($Consulta);
     ...
     Foreach($TMP as $Tabela) {
       ...
       // IATA
       If ( Texto_SEO($Tabela['IATA']) == Texto_SEO($POST['IATA']) ) Return Array('RT' => 'Erro', 'Info' => 'O IATA Code [ '.$POST['IATA'].' ] já existe na base de dados para '.$Objeto.' [ <a href="'.URL_Link($Modulo, 'Editar', '', 'aeID='.Formata_Codigo($Tabela['Codigo'])).'" id="Tema">'.$Tabela['IATA_ID'].'</a> ]');
       // IATA Nome
       If ( Texto_SEO($Tabela['IATA_Nome']) == Texto_SEO($POST['IATA_Nome']) ) Return Array('RT' => 'Erro', 'Info' => 'O IATA Name [ '.$POST['IATA_ID'].' ] já existe na base de dados para '.$Objeto.' [ <a href="'.URL_Link($Modulo, 'Editar', '', 'aeID='.Formata_Codigo($Tabela['Codigo'])).'" id="Tema">'.$Tabela['IATA_ID'].'</a> ]');
     }
   }

// EXECUCAO
$SQL               = 'INSERT INTO '.TAB_Aeroportos.'
...
Function Aeroporto_Editar($POST, $_ID = 0) { Global $_DB, $_TP, $_SIS, $_Mundo; array_map('trim', $POST);
...
// DUPLICIDADE
$SQL               = 'SELECT Codigo, Codigo_ID AS ID, Nome, IATA, IATA_Nome, IATA_ID FROM '.TAB_Aeroportos.' WHERE ( Nome = $1 OR IATA = $2 OR IATA_ID = $3'.( $ID ? ' OR ID = $4' : '' ).' ) AND Codigo != $5';
...
Function Aeroporto_OPT($Selecao = NULL, $Limite = True, $Campos = 'Codigo', $Mascara = '$Codigo', $Value = 'Codigo', $Ordenacao = '', $Opcao_Nula = '0', $Status = False) { Global $_DB;

$Modulo            = 'Aeroportos';
$Objeto            = 'o aeroporto';
$SQL_Ordem         = $Ordenacao ? $Ordenacao : 'Codigo DESC';
$SQL_Base          = 'SELECT '.$Campos.' FROM '.TAB_Aeroportos.( $Status ? ' WHERE Reg_Status = "'.$Status.'"' : '' );
$Consulta          = $_DB->Consulta($SQL_Base.' ORDER BY '.$SQL_Ordem);
$Lista_Total       = $_DB->Registros($Consulta);
$Lista_Limite      = defined('AEROPORTO_Lista_Limite') ? (int)AEROPORTO_Lista_Limite : ( defined('PAG_Lista_Limite') ? (int)PAG_Lista_Limite : 0 );
