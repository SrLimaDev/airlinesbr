...
/*** ACOES INDIVIDUAIS ***/
Function Jogador($ID = 0, $Acao = 'Dados') { Global $_DB;

$Modulo            = 'Jogadores';
$Objeto            = 'o jogador';

// IDENTIFICACAO
$SQL               = 'SELECT * FROM '.TAB_Jogadores.' WHERE Codigo = $1';
...
If ($_DB->Registros($Consulta))
   { $RG           = $_DB->Dados($Consulta);
     ...
     Switch($Acao) {
       ...
       Case 'Ativar':
            If ( $Status == 'Inativo' )
               { $SQL        = 'UPDATE '.TAB_Jogadores.' SET Reg_Status = "Ativo" WHERE Codigo = $1';
       ...
       Case 'Desativar':
            If ( $Status == 'Ativo' )
               { $SQL        = 'UPDATE '.TAB_Jogadores.' SET Reg_Status = "Inativo" WHERE Codigo = $1';
                               $_DB->Consulta($SQL, Array('$1' => $ID));
                 $Log        = Array('Modulo' => $Modulo, 'Evento' => $_SESSION['SIS']['L']['Nome'].' ativou o registro d'.$Objeto.' [ '.$Registro.' ]');
       ...
       // Excluir
       Case 'Excluir':
            $SQL   = 'DELETE FROM '.TAB_Jogadores.' WHERE Codigo = $1';
                     $_DB->Consulta($SQL, Array('$1' => $ID));
...
/*** ACOES MULTIPLAS ***/
Function Jogador_Acoes($IDs = Array(), $Acao = 'Dados') {
...
Function Jogador_Cadastrar($POST) { Global $_DB, $_TP, $_SIS; array_map('trim', $POST);
...
// Doc Zero
$DocZero           = defined('JOGADOR_'.$Doc_Campo.'_Zero') ? constant('JOGADOR_'.$Doc_Campo.'_Zero') : False;
...
// DUPLICIDADE
$SQL               = 'SELECT Codigo, Codigo_ID AS ID, Tipo, Tratamento, Email, '.$Doc_Campo.', Acesso_Usuario FROM '.TAB_Jogadores.' WHERE Tratamento = $1 OR Email = $2 OR '.$Doc_Campo.' = $3 '.( $ID ? ' OR ID = $4' : '' ).( $Acesso_Usuario ? ' OR Acesso_Usuario = $5' : '' );
     ...
     Foreach($TMP as $Tabela) {
       ...
       If ( !$DocZero ) // Verifica se o valor nao for zero e, se for, se nao estiver permititdo
          { $Comparar        = ( $Tabela['Tipo'] == 'Pessoa Jurídica' && $Doc_Campo == 'CNPJ' ) || ( $Tabela['Tipo'] != 'Pessoa Jurídica' && $Doc_Campo == 'CPF' ) ? True : False; // Necessario para evitar comparacao de CPF de pessoa fisica com CPF de responsavel de pessoa juridica
            If ( $Comparar && $Tabela[$Doc_Campo] == $Doc_Valor ) Return Array('RT' => 'Erro', 'Info' => 'O '.$Doc_Campo.' [ '.$Doc_Formato.' ] já existe na base da dados para o jogador [ <a href="'.URL_Link($Modulo, 'Editar', '', 'cID='.Formata_Codigo($Tabela['Codigo'])).'" id="Tema">'.$Tabela['Tratamento'].'</a> ]');
          }
       // Email
       If ( Texto_SEO($Tabela['Email']) == Texto_SEO($POST['Email']) ) Return Array('RT' => 'Erro', 'Info' => 'O e-mail [ '.$POST['Email'].' ] já existe na base da dados para o jogador [ <a href="'.URL_Link($Modulo, 'Editar', '', 'cID='.Formata_Codigo($Tabela['Codigo'])).'" id="Tema">'.$Tabela['Tratamento'].'</a> ]');
       // Usuario
       If ( $Acesso_Usuario )
...
Function Jogador_Editar($POST, $_ID = 0) { Global $_DB, $_TP, $_SIS; array_map('trim', $POST);
...
// DUPLICIDADE
$SQL               = 'SELECT Codigo, Codigo_ID AS ID, Tipo, Tratamento, Email, '.$Doc_Campo.', Acesso_Usuario FROM '.TAB_Jogadores.' WHERE ( Tratamento = $1 OR Email = $2 OR '.$Doc_Campo.' = $3 '.( $ID ? ' OR ID = $4' : '' ).( $Acesso_Usuario ? ' OR Acesso_Usuario = $5' : '' ).' ) AND Codigo != $6';
...
Function Jogador_OPT($Selecao = NULL, $Limite = True, $Campos = 'Codigo', $Mascara = '$Codigo', $Value = 'Codigo', $Ordenacao = '', $Opcao_Nula = '0', $Status = False) { Global $_DB;

$Modulo            = 'Jogadores';
$Objeto            = 'o jogador';
$SQL_Ordem         = $Ordenacao ? $Ordenacao : 'Codigo DESC';
$SQL_Base          = 'SELECT '.$Campos.' FROM '.TAB_Jogadores.( $Status ? ' WHERE Reg_Status = "'.$Status.'"' : '' );
$Consulta          = $_DB->Consulta($SQL_Base.' ORDER BY '.$SQL_Ordem);
$Lista_Total       = $_DB->Registros($Consulta);
$Lista_Limite      = defined('JOGADOR_Lista_Limite') ? (int)JOGADOR_Lista_Limite : ( defined('PAG_Lista_Limite') ? (int)PAG_Lista_Limite : 0 );
...
Function Jogador_Nome($Dados = 0, $Retorno = 'Tratamento') { Global $_DB;

// Variaveis
$Dados             = is_array($Dados) ? $Dados : Jogador((int)$Dados);
